        ...
        if isinstance(self, ExternKernelSchedulerNode):
            ...
            if op is not None:
                ...
                with FakeTensorMode(), FlopCounterMode(
                    ...
                    fake_inputs = [
                        ir_node_to_tensor(input, guard_shape=False) for input in self.node.inputs
                    ]
...
class Scheduler:
    ...
    def codegen(self):
        for node in self.nodes:
            log.debug("Generating code for node %s with estimated runtime %f", node.get_name(),node.get_estimated_runtime())
